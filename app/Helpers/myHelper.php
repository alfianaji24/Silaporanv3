...
// Helper functions for login info
if (!function_exists('getLastLoginIP')) {
    function getLastLoginIP($userId)
    {
        try {
            $loginLog = \App\Models\UserLoginLog::where('user_id', $userId)
                ->where('is_successful', true)
                ->orderBy('login_time', 'desc')
                ->first();
            
            return $loginLog ? $loginLog->ip_address : null;
        } catch (\Exception $e) {
...
if (!function_exists('getLastDeviceInfo')) {
    function getLastDeviceInfo($userId)
    {
        try {
            ...
            if ($loginLog) {
                $deviceInfo = $loginLog->device_type;
                if ($loginLog->browser && $loginLog->browser_version) {
                    $deviceInfo .= ' - ' . $loginLog->browser . ' ' . $loginLog->browser_version;
                }
                return $deviceInfo;
            }
...
if (!function_exists('getLastLoginTime')) {
    function getLastLoginTime($userId)
    {
        try {
            ...
            return $loginLog ? $loginLog->login_time->diffForHumans() : null;
        } catch (\Exception $e) {
