<form action="{{ route('statuskaryawan.update', ['kode_status_karyawan' => Crypt::encrypt($status_karyawan->kode_status_karyawan)]) }}" method="POST" id="formStatusKaryawan">
    @csrf
    @method('PUT')
    <x-input-with-icon label="Kode Status Karyawan" name="kode_status_karyawan" icon="ti ti-barcode" :value="$status_karyawan->kode_status_karyawan" maxlength="1" placeholder="Contoh: K (Maksimal 1 karakter)" />
    <x-input-with-icon label="Nama Status Karyawan" name="nama_status_karyawan" icon="ti ti-user-check" :value="$status_karyawan->nama_status_karyawan" maxlength="50" placeholder="Contoh: Kontrak (Maksimal 50 karakter)" required />
