...
<head>
    ...
    <style>
        @page {
            size: A3 landscape;
            /* Higher size for Format 2 since it's very wide */
            margin: 10mm;
        }
        ...
        .datatable3 {
            ...
            /* Required for perfect sticky alignment with borders */
            border-spacing: 0;
            /* Ensures cells touch exactly without gaps */
            font-size: 9px;
            min-width: 100%;
        }
        ...
        .datatable3 td {
            ...
            font-size: 11px;
            /* Uniform font size for better visibility */
        }
        ...
        .datatable3 th {
            ...
            /* Shared border simulation */
            background-clip: padding-box;
            border: none;
            height: 45px;
            /* Increased height for larger font labels */
            line-height: 1.1;
        }

        /* Sticky header rows - precisely calculated offsets matching 45px height */
        .datatable3 thead tr:nth-child(1) th {
            top: 0;
            z-index: 24;
        }
        ...
        .datatable3 thead tr:nth-child(4) th {
            top: 135px;
            z-index: 21;
        }
...
<body>
    ...
    <div class="table-responsive">
        <table class="datatable3">
            <thead>
                @php
                $showDenda = !empty($generalsetting->denda);
                $showPotongan = $generalsetting->status_potongan_jam == 1;
                $showLembur = !empty($generalsetting->lembur);
                $showIstirahat = $generalsetting->absen_istirahat == 1;
                $dailyColumnCount = 2 + ($showIstirahat ? 2 : 0) + ($showLembur ? 1 : 0) + ($showPotongan ? 1 : 0) + ($showDenda ? 1 : 0);
                @endphp
                <tr>
                    <th rowspan="4" class="sticky-col first-col">No</th>
                    <th rowspan="4" class="sticky-col second-col">Nama</th>
                    <th rowspan="4" class="sticky-col third-col">Jabatan</th>
                    <th colspan="{{ $jmlhari * $dailyColumnCount }}">Tanggal</th>
                    @if($showDenda)
                ...
                <tr>
                    ...
                    @while (strtotime($tanggal_presensi) <= strtotime($periode_sampai))
                        <th colspan="{{ $dailyColumnCount }}">{{ getHari(date('Y-m-d', strtotime($tanggal_presensi))) }}</th>
                        @php
                        $tanggal_presensi = date('Y-m-d', strtotime('+1 day', strtotime($tanggal_presensi)));
                        @endphp
                        @endwhile
                        <th rowspan="3">Hadir <br> (hr)</th>
                        <th rowspan="3">Izin<br> (hr)</th>
                        <th rowspan="3">Sakit<br> (hr)</th>
                        <th rowspan="3">Alfa<br> (hr)</th>
                        <th rowspan="3">Libur<br> (hr)</th>
                ...
                <tr>
                    ...
                    @while (strtotime($tanggal_presensi) <= strtotime($periode_sampai))
                        <th colspan="{{ $dailyColumnCount }}">{{ date('d', strtotime($tanggal_presensi)) }}</th>
                        @php
                        $tanggal_presensi = date('Y-m-d', strtotime('+1 day', strtotime($tanggal_presensi)));
                        @endphp
                        @endwhile
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="sticky-col second-col">{{ $d['nama_karyawan'] }}</td>
                    <td class="sticky-col third-col">{{ $d['nama_jabatan'] }}</td>
                    @php
                    ...
                    @while (strtotime($tanggal_presensi) <= strtotime($periode_sampai))
                        ...
                        @php
                        // Jadwal column removed for format 2
                        @endphp
                        @if ($row_p['status'] == 'h')
                        @php
                        $jml_hadir++;
